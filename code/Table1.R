...
METRICS_TO_RUN <- if (length(args) >= 1L) args[1] else names(DONOR_FILES)
YEARS_TEST <- if (length(args) >= 2L) as.integer(strsplit(args[2], ",")[[1]]) else c(2017L, 2018L)

# Optional 3rd arg: turbine range as "start:end" (e.g. "1:11"). Default = all.
TURBINE_RANGE <- if (length(args) >= 3L) {
  parts <- as.integer(strsplit(args[3], ":")[[1]])
...
run_metric <- function(metric_name, donor_path) {
  ...
  detail_path <- file.path(RESULTS_DIR, sprintf("Table1_%s%s_detail.csv", metric_name, RANGE_TAG))
  summary_path <- file.path(RESULTS_DIR, sprintf("Table1_%s%s_summary.csv", metric_name, RANGE_TAG))
...
main <- function() {
  ...
  fwrite(all_summary, file.path(RESULTS_DIR, sprintf("Table1_summary_all_metrics%s.csv", RANGE_TAG)))
}
