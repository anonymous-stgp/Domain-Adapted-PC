...
OUTPUT_DETAIL <- file.path(RESULTS_DIR, paste0("Table2_G_random_forest_detail", SETUP_SUFFIX, ".csv"))
OUTPUT_SUMMARY <- file.path(RESULTS_DIR, paste0("Table2_G_random_forest_summary", SETUP_SUFFIX, ".csv"))
...
read_geo_donor_table <- function(path) {
  ...
  if (all(c("donor", "geo_distance") %in% names(dt))) {
    ...
    dt <- dt[target %in% TEST_IDS & donor %in% TRAIN_IDS]
  ...
  long <- long[target %in% TEST_IDS & donor %in% TRAIN_IDS]
  setorder(long, target, geo_distance, donor)
  long
}

get_top_k_donors <- function(dt, target_id, k) {
  sub <- dt[target == target_id & donor %in% TRAIN_IDS & donor != target_id]
  head(sub$donor, k)
}

build_pooled_training <- function(donor_ids) {
  donor_ids <- donor_ids[donor_ids %in% TRAIN_IDS]
...
main <- function() {
  ...
  for (target_id in targets) {
    ...
    for (year in TEST_YEARS) {
      cat("G_random_forest DFP - Turbine", target_id, "Year", year, "\n")
