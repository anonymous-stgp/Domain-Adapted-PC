...
Table2_P_GNN_dfp.py
====================
DFP STGNN for wind turbines.
- TEST_IDS  = range(38,45) + range(61,67)
- TRAIN_IDS = all other turbines
- Trains one model on all TRAIN_IDS 2017 data, tests on each TEST_ID
- Package-friendly paths
- Incremental save + resume
- Year argument: python code/Table2_P_GNN_dfp.py 2017
"""
...
DETAIL_FILE  = RESULTS_DIR / "Table2_P_GNN_dfp_detail.csv"
SUMMARY_FILE = RESULTS_DIR / "Table2_P_GNN_dfp_summary.csv"
...
ID_TO_IDX = {tid: i for i, tid in enumerate(ALL_IDS_SORTED)}
POS_ARR   = np.array([POS[tid] for tid in ALL_IDS_SORTED], dtype=np.float64)
...
class FixedKNNSubgraphDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return Data(
            node_seq=torch.from_numpy(node_seq), y=torch.from_numpy(y),
            mask=torch.from_numpy(mask),
            edge_index=self.edge_index[focus_tid], edge_attr=self.edge_attr[focus_tid],
...
def train_dfp_model():
    ...
    for t in TRAIN_IDS:
        ...
        if df is None:
            print(f"  [WARN] Missing Turbine{t}_2017.csv", flush=True)
            continue
        train_data[t] = preprocess_df(df)

    mu, sd = compute_scaler(train_data)
    ds = FixedKNNSubgraphDataset(train_data, mu, sd, list(train_data.keys()),
...
def eval_target(model, mu, sd, target_tid, year):
    ...
    # use TRAIN_IDS as neighbors (+ target itself)
    avail = [t for t in TRAIN_IDS if t in data_by_tid]
    nodes = knn_ids_within_set(target_tid, avail + [target_tid], K_NEIGHBORS)
    step_ns = np.int64(pd.Timedelta(minutes=FREQ_MIN).value)

    arrX, arry, tarr = {}, {}, {}
    for tid in nodes:
        if tid not in data_by_tid:
            continue
        df = data_by_tid[tid]
        tvals = df[TIME_COL].astype("int64").values.astype(np.int64)
        tarr[tid] = tvals
    ...
    nodes = [n for n in nodes if n in arrX]
    if not nodes or target_tid not in arrX:
        return float("nan"), 0.0
...
def main():
    ...
    print(f"TEST_IDS: {TEST_IDS}", flush=True)
    print(f"TRAIN_IDS: {len(TRAIN_IDS)} turbines", flush=True)

    done_keys = load_done_keys()

    years_needed = [y for y in TEST_YEARS
                    if any(f"{t}|{y}" not in done_keys for t in TEST_IDS)]
    ...
    # Train once on all TRAIN_IDS
    print("\n=== Training DFP model on all TRAIN_IDS ===", flush=True)
    try:
        model, mu, sd, fit_time = train_dfp_model()
    except Exception as e:
        print(f"[ERROR] train: {e}", flush=True)
        return
    print(f"  fit_time={fit_time:.1f}s", flush=True)

    # Evaluate on each test turbine
    for target_tid in TEST_IDS:
        for year in TEST_YEARS:
            pair_key = f"{target_tid}|{year}"
            if pair_key in done_keys:
                print(f"[SKIP] Turbine {target_tid} Year {year}", flush=True)
                continue

            print(f"\nEvaluating Turbine {target_tid} Year {year} ...", flush=True)
            try:
            ...
            print(f"  -> Saved. RMSE={rmse_val:.4f}", flush=True)
