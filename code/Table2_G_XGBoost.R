...
DETAIL_FILE  <- file.path(RESULTS_DIR, "Table2_G_XGBoost_detail.csv")
SUMMARY_FILE <- file.path(RESULTS_DIR, "Table2_G_XGBoost_summary.csv")
...
read_geo_donor_table <- function(path) {
  ...
  # wide format: donor1/donor_1 + score1/score_1
...
top_k_geo_donors <- function(geo_df, target_id, k) {
  sub <- geo_df[target == target_id & donor != target_id]
...
targets <- sort(unique(geo_df$target))
...
cat(sprintf("[INFO] Starting G_XGBoost — %d targets x %d years\n", n_targets, length(TEST_YEARS)))
...
for (t_idx in seq_along(targets)) {
  ...
  # build pooled training data from donors
  frames <- list()
  for (d in donors) {
