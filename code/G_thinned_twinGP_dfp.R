...
DETAIL_PATH  <- file.path(RESULTS_DIR, sprintf("Table2_G_thinned_twinGP%s_detail.csv",  SETUP_SUFFIX))
SUMMARY_PATH <- file.path(RESULTS_DIR, sprintf("Table2_G_thinned_twinGP%s_summary.csv", SETUP_SUFFIX))
...
get_top_k_donors <- function(dt, target_id, k, allowed_donors) {
  sub <- dt[target == target_id & donor != target_id & donor %in% allowed_donors]
  head(sub$donor, k)
}
...
main <- function() {
  ...
  turbine_ids <- TEST_IDS

  cat("Model      : G_thinned_twinGP (DFP, geographic donors, K =", K, ", ensemble)\n")
  cat("Test turbines :", length(turbine_ids), "\n")
  cat("Train pool    :", length(TRAIN_IDS), "turbines\n")
  cat("Years         :", paste(YEARS_TEST, collapse = ", "), "\n\n")

  for (target_id in turbine_ids) {
    donors <- get_top_k_donors(geo_dt, target_id, K, TRAIN_IDS)
