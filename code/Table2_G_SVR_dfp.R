...
# --- year argument: Rscript --vanilla code/Table2_G_SVR_dfp.R 2017
...
DONOR_FILE     <- file.path(PROCESSED_DIR, paste0("matching_geographic_distance", SETUP_SUFFIX, ".csv"))
OUTPUT_DETAIL  <- file.path(RESULTS_DIR,   paste0("Table2_G_SVR_detail",  SETUP_SUFFIX, year_suffix, ".csv"))
OUTPUT_SUMMARY <- file.path(RESULTS_DIR,   paste0("Table2_G_SVR_summary", SETUP_SUFFIX, year_suffix, ".csv"))
...
read_geo_donor_table <- function(path) {
  ...
  # wide format: target, donor1, score1, donor2, score2 ...
  df          <- as.data.frame(dt)
  donor_cols  <- grep("^donor[0-9]", names(df), value = TRUE)
  score_cols  <- grep("^score[0-9]", names(df), value = TRUE)
...
get_top_k_donors <- function(dt, target_id, k) {
  sub <- dt[target == target_id & donor != target_id & donor %in% TRAIN_IDS]
  head(sub$donor, k)
}
...
main <- function() {
  ...
  targets <- intersect(sort(unique(geo_dt$target)), TEST_IDS)
  ...
  cat(sprintf("[INFO] Starting G_SVR (DFP) — %d targets x %d years\n",
  ...
  for (t_idx in seq_along(targets)) {
    ...
    for (year in TEST_YEARS) {
      ...
      cat(sprintf("[%d/%d] G_SVR DFP - Turbine %d Year %d\n",
