...
OUTPUT_DETAIL  <- file.path(RESULTS_DIR, paste0("Table2_TF_thinnedSV_detail",  SETUP_SUFFIX, ".csv"))
OUTPUT_SUMMARY <- file.path(RESULTS_DIR, paste0("Table2_TF_thinnedSV_summary", SETUP_SUFFIX, ".csv"))
...
load_turbine_year <- function(turbine_id, year) {
  path <- file.path(DATA_DIR, sprintf("Turbine%d_%d.csv", turbine_id, year))
...
create_thinned_bins <- function(dataX, dataY, thinning_number) {
  ...
  for (i in seq_len(thinning_number)) {
    idx      <- seq(i, n, by = thinning_number)
    bins[[i]] <- list(X = dataX[idx, , drop = FALSE], y = dataY[idx])
  }
  bins
}

# --- fit once on donor 2017 data, return fit_obj ---
...
# --- predict for a given (target, year) using already-fitted model ---
...
run_metric <- function() {
  ...
  targets    <- intersect(sort(unique(donor_dt$target)), TEST_IDS)
  ...
  cat(sprintf("[INFO] Starting TF_thinnedSV (DFP) — %d targets x %d years\n",
  ...
  for (t_idx in seq_along(targets)) {
    ...
    donors    <- intersect(donors, TRAIN_IDS)
    donors    <- donors[seq_len(min(K, length(donors)))]
    if (!length(donors)) next
    
    # skip if all years done
    if (all(sapply(YEARS_TEST, function(y) sprintf("%d|%d", target_id, y) %in% done_keys))) {
    ...
    for (test_year in YEARS_TEST) {
      yr       <- as.character(test_year)
      pair_key <- sprintf("%d|%d", target_id, test_year)
      if (pair_key %in% done_keys || !length(pred_lists[[yr]])) next
